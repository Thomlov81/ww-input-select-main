...
export default {
    ...
    emits: ['remove-multiselect-value', 'clear-all-multiselect-values'],
    setup(props, { emit }) {
...
- \`placeholder\`: The placeholder text for the select trigger.`;
        ...
        const chipClearIconStyle = computed(() => {
            return {
                width: props.content.chipIconSize,
                color: props.content.chipIconColor,
                display: 'flex',
        ...
        const selectedChipsCount = computed(() => {
            return selectedChips.value?.length || 0;
        });
        ...
        const handleClearAllClick = (event) => {
            event.stopPropagation();
            emit('clear-all-multiselect-values');
        };
